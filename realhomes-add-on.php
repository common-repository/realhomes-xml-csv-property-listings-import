...
function realhomes_wpai_addon_get_page_by_title( $title, $return_type = 'OBJECT', $post_type = 'post' ) {
	$posts = get_posts(
		array(
			'post_type'              => $post_type,
			'title'                  => $title,
...
$realhomes_addon->add_field( 'REAL_HOMES_property_lot_size', 'Lot Size', 'text', null, 'Only digits, example: 3000' );

$realhomes_addon->add_field( 'REAL_HOMES_property_lot_size_postfix', 'Lot Size Postfix', 'text', null, 'Example: sq ft' );
...
$realhomes_addon->add_field( 'REAL_HOMES_energy_class', 'Energy Class', 'radio', array(
	'none' 	=> 'None',
	'A+' 	=> 'A+',
	'A' 	=> 'A',
	'B' 	=> 'B',
...
$realhomes_addon->add_field( 'REAL_HOMES_energy_performance', 'Energy Performance', 'text', null, 'Example Value: 100 kWh/m²a' );

$realhomes_addon->add_field( 'REAL_HOMES_epc_current_rating', 'EPC Current Rating', 'text', null, 'Example Value: 83' );

$realhomes_addon->add_field( 'REAL_HOMES_epc_potential_rating', 'EPC Potential Rating', 'text', null, 'Example Value: 94' );
...
$realhomes_addon->add_options( null, 'Advanced Settings', array(
	...
	$realhomes_addon->add_field( 'inspiry_video_group_images', 'Video Images', 'text', null, 'If importing multiple images, use a comma-delimited list.' ),
	$realhomes_addon->add_field( 'inspiry_video_group_videos', 'Video URLs or Filenames', 'text', null, 'Filenames will only work if the video exists in the Media Library. If importing multiple videos, use a comma-delimited list.' ),
	$realhomes_addon->add_field( 'inspiry_video_group_titles', 'Video Titles', 'text', null, 'If importing multiple titles, use a comma-delimited list.' ),
...
function realhomes_addon_import( $post_id, $data, $import_options, $article ) {
	...
	$fields = array(
		...
		'REAL_HOMES_property_lot_size_postfix',
		...
		'REAL_HOMES_energy_class',
		'REAL_HOMES_energy_performance',
		'REAL_HOMES_epc_current_rating',
		'REAL_HOMES_epc_potential_rating'
	);
	...
	if ( empty( $article['ID'] ) or $realhomes_addon->can_update_meta( $field, $import_options ) ) {
		...
		$post = realhomes_wpai_addon_get_page_by_title( $data[$field], 'OBJECT', $post_type );
		...
		if ( !empty($post) ) {
			...
			if ( $post = realhomes_wpai_addon_get_page_by_title( $data[$field], 'OBJECT', $post_type ) ) {
	...
	$fields = array(
		'inspiry_video_group_images',
		'inspiry_video_group_titles',
		'inspiry_video_group_videos'
	);

	$video_real_field = 'inspiry_video_group';
	$video_array      = array();
	$i                = '0';

	$image_data = explode( ',', $data['inspiry_video_group_images'] );
	$title_data = explode( ',', $data['inspiry_video_group_titles'] );
	$video_data = explode( ',', $data['inspiry_video_group_videos'] );
	...
	foreach ( $image_data as $k => $v ) {
		$image_id = PMXI_API::upload_image( $post_id, trim( $v ), 'yes', $logger, true );
		$title    = trim( $title_data[ $k ] );

		$video_array[ $i ]['inspiry_video_group_image'] = [ $image_id ];
		$video_array[ $i ]['inspiry_video_group_title'] = $title;
		$video_array[ $i ]['inspiry_video_group_url']   = trim( $video_data[ $k ] );
		$i++;
	}

	update_post_meta( $post_id, $video_real_field, $video_array );
}